...
func (group *Group) GET(path string, handlers ...Handler) Router {
	route := group.app.register(MethodGet, getGroupPath(group.prefix, path), handlers...)
	// Add head route
	headRoute := route
	group.app.addRoute(MethodHead, &headRoute)
	return group
}
